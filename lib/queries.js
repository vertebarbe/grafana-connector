...
exports.results = function (cfg, params) {
    ...
    for (let def of defs) {
        ...
        if (!hideEmpty || datapoints.length > 0 || logQuery) {
            ...
            const nodeFields = [{name:"id", type:"string", config:{}, values:query[0]["nodes"].map(({id}) => id)},{name:"title", type:"string", config:{}, values:query[0]["nodes"].map(({title}) => title)}];
            const edgefields = [{name:"id", type:"string", config:{}},{name:"source", type:"string", config:{}, values:query[0]["nodes"].map(({source}) => source)},{name:"target", type:"string", config:{}, values:query[0]["nodes"].map(({target}) => target)},{name:"mainstat", type:"string", config:{}, values:query[0]["nodes"].map(({mainstat}) => mainstat)}];

...
exports.results = function (cfg, params) {
    ...
    for (let def of defs) {
        ...
        if (!hideEmpty || datapoints.length > 0 || logQuery) {
            ...
            const nodeFields = [{name:"id", type:"string", config:{}, values:datapoints[0]["nodes"].map(({id}) => id)},{name:"title", type:"string", config:{}, values:datapoints[0]["nodes"].map(({title}) => title)}];
            const edgeFields = [{name:"id", type:"string", config:{}, values:datapoints[0]["edges"].map(({id}) => id)},{name:"source", type:"string", config:{}, values:datapoints[0]["edges"].map(({source}) => source)},{name:"target", type:"string", config:{}, values:datapoints[0]["edges"].map(({target}) => target)},{name:"mainstat", type:"string", config:{}, values:datapoints[0]["edges"].map(({mainstat}) => mainstat)}];
            const frameMetadata = {preferredVisualisationType: 'nodeGraph'};
            const edgeFrame = {name: "Edges", refId: null, fields: edgeFields, meta: frameMetadata};

            const data = {"nodes": nodeFrame, "edges": edgeFrame}
            const result = {target: def.target}
            if (def.target == "nodes") {
                response.push({target: def.target, name: "Nodes", refId: null, fields: nodeFields, meta: frameMetadata});
            }
            if (def.target == "edges") {
                response.push({target: def.target, name: "Edges", refId: null, fields: edgeFields, meta: frameMetadata});
            }
        }
    }
